...
function officialPage(url) {
  try { const u = new URL(url); return u.protocol === 'https:' && ['8ballpool.com', 'www.8ballpool.com'].includes(u.hostname); }
  catch { return false; }
}
...
class ShopReturnGate {
  constructor() { this.since = null; this.url = null; this.used = false; }
  reset() { this.since = null; this.url = null; }
  observe(url, shop, now) {
    if (this.used) return false;
    if (!officialPage(url) || !shop) { this.reset(); return false; }
    if (this.url !== url || this.since === null) { this.since = now; this.url = url; return false; }
