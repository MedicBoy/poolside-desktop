...
`test/fixtures/vision-corpus.json` is a structured set of 15 text layouts: eight derived from recorded screens — the seven recognisable screens, plus one recorded lobby whose hint text is misread, which must rightly change nothing — and seven failure modes that have no screenshot at all (a blank transition, a mid-load partial, a dialog, the shop, non-English wording, a wrong-aspect surface, and a window captured at half density). Each frame lists the wording it shows, where that wording appears, and how confident a recogniser was in reading it. `test/vision-corpus.test.cjs` runs every frame through the matching logic and asserts the state it claims, so changing the rules — or the text they read — fails here.
