...
A saved **run report** is a separate, deliberately narrower record: it is written only when the operator asks for it, it is not a bundle of whatever happened to be in memory, and it excludes route credentials, browser profiles, page text and captured images. It includes account names, because a record of who played whom without names is not a record, and it says so inside the file.
