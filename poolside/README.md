...
`test/fixtures/vision-corpus.json` is a structured set of 14 text layouts: the seven recorded screens, plus the failure modes that have no screenshot — a blank transition, a mid-load partial, a dialog, the shop, non-English wording, a wrong-aspect surface, and a window captured at half density. Each frame lists the wording it shows, where that wording appears, and how confident a recogniser was in reading it. `test/vision-corpus.test.cjs` runs every frame through the matching logic and asserts the state it claims, so changing the rules — or the text they read — fails here.
