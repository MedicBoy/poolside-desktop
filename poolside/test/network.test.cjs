...
test('IP check uses the supplied session and omits credentials', async () => {
  const session = { async fetch(url, options) {
...
test('IP check rejects service errors, malformed addresses and oversized responses', async () => {
  for (const response of [new Response('', { status: 503 }), new Response('{"ip":"<script>"}'), new Response('x'.repeat(1025)), new Response('not json')]) {
