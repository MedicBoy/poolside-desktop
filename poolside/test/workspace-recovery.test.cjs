...
test('the dashboard surface previews, confirms, restores and says what it did', async () => {
  await withRoot(async root => {
    ...
    assert.match(asked.detail, /kept as the previous copy first, so this can be undone/);
    assert.equal(saved.length, 1, "the restore goes through the application's own save");
    assert.equal(saved[0].accounts.length, 1);
