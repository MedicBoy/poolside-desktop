...
| Field                | Value                                                                                                                |
| :------------------- | :------------------------------------------------------------------------------------------------------------------- |
| Repository root      | `C:\Users\nicho\OneDrive\Desktop\Coding` (git root; covers `poolside/`, `ROADMAP.md`, `INCOMPLETE_WORK.md`, reviews) |
| Application root     | `C:\Users\nicho\OneDrive\Desktop\Coding\poolside`                                                                    |
| Branch               | `master` (trunk-based)                                                                                               |
| HEAD commit          | `8420e24` — documentation baseline at M5                                                                             |
| Feature commit       | `5054e82` — _feat(m5): settings UI generated from the configuration schema_                                          |
...
| Untracked            | See current `git status`; repository-state details in this snapshot may have changed since preparation               |
| App version          | `0.1.0`                                                                                                              |
...
The FSM is the **only writer** of session state. Nothing else may assign it; a feature raises a _named event_ and the machine decides whether it applies.
...
Failure detection drives the FSM; the _policy_ is a value, so it is testable without a clock and without Electron.
...
| Recovery mechanism | injected — supervision _decides_, `windows.cjs` _acts_                                                      |
| Feed discipline    | only a _degradation_ writes an activity entry, or the feed floods                                           |
...
| :--------------------- | :------------------------------------------------------------------------------------------------------------------------------------ |
| `config-schema.cjs`    | _what exists_ — field lists, section nesting, required flags, defaults, bounds, labels. Declares only; delegates every rule.          |
| `config-walk.cjs`      | _how to walk a declaration_ — collects every problem at once and returns the usable value; owns the errors-vs-dropped rule.           |
| `config-validator.cjs` | _which declarations exist at a boundary_ and what their problems mean. Three entry points: settings, account config, session profile. |
...
No rule is implemented twice: identity checks _are_ `identity-fields.validateField`, route checks _are_ `proxy.parseProxySpec`, and the venue list lives once and is re-exported. A parity suite cross-references the declaration against the structures that are actually stored, over a 21-value corpus asserted verdict-for-verdict against the legacy `model.settings` path. The boundary runs at two moments — `settings:save` (refuse before storage) and session launch (refuse before executing).
...
| Read     | `timeline-query.cjs`                               | `index`, `query`, `failures`, `summarise`. Throws on nothing — this is the module a caller reaches for _while_ something is failing.                                                                                                                                                                                                                                                                                                         |
...
| 3.3 | **Match completion and outcome tracking**          | Forfeit/leave flows, outcome detection, repeated-match limits, and win/loss state                                                                                    | **Not built.** No outcome logic exists anywhere in the codebase.                                                                                 |
| 3.4 | **Device-fingerprint controls**                    | Per-account HWID, install-date, canvas, WebGL, audio, and related fingerprint settings                                                                               | **Not built.** Existing identity configuration covers user agent, locale, timezone, viewport, and color scheme through documented Electron APIs. |
| 3.5 | **Matchmaking-aware routing**                      | Selecting per-session routes according to pool, region, or opponent conditions                                                                                       | **Not built.** Current route support applies and health-checks a configured route only.                                                          |
...
| 3.8 | **Challenge handling and reference compatibility** | Rate-limit handling, challenge/captcha flows, and recovery needed to match the reference workflow                                                                    | **Not built.**                                                                                                                                   |
...
- **A layout and coordinate analyzer.** A capture pipeline that owns the arithmetic between page CSS pixels, Electron's DIP capture rectangle, the returned image's true density, and the resized buffer a recogniser reads — and that _reports_ a density mismatch instead of silently cropping plausible pixels.
...
| B3  | **`session.fromPartition(id)` resurrected a deleted profile directory** — an explicit profile delete appeared to fail because the directory was back                              | Resolving a partition by id _creates_ the partition and its directory. The delete path asked for a session it did not already hold, recreating what had just been removed.                                                         | Never ask for a session you do not already hold; deleting the files _is_ the deletion. The desktop suite now asserts the directory is gone afterwards.                                                                      | Desktop suite profile-lifecycle assertion                                                                                          |
| B4  | **Profile generation counter inflated on every open** — "generation" measured nothing                                                                                             | Chromium creates a partition directory **lazily**, so `fs.existsSync`-based generation read _absent_ as _created_, incrementing on every open.                                                                                     | Generation tracks **establishment**, not directory presence: a persisted `established` flag decides.                                                                                                                        | `test/profile-manager.test.cjs`                                                                                                    |
...
| B11 | **`compile(null)` and `sessionLayer([null])` threw**                                                                                                                              | A default parameter covers only `undefined`, not `null`; and mapping a raw array put `null` into `account.id`.                                                                                                                     | Explicit guards plus a `records()` filter in every layer. These modules are the ones a caller reaches for _while_ something is already failing, so "nothing throws" is a test, not a hope.                                  | "nothing here throws" suites                                                                                                       |
...
| **No per-session storage quota exists** — no `setQuota`/`storageQuota` in the Session API                                                                                                             | A configurable ceiling can only be _measured and reported_, never enforced. `session.getCacheSize()` is the HTTP disk cache (0 for a fresh partition); `navigator.storage.estimate()` reports Chromium's own ~1.5 GB allowance |
...
### 5.1 Current integration coverage

This section is **partial, and is unfinished**. It is split into:

- **5.2 — input emission and matchmaking coordination: unfinished.** The target workflow requires an input driver, live matchmaking-state observation, coordination, and display-buffer integration. Those components and their integration map have not been implemented
...
### 5.2 Input emission and matchmaking coordination — unfinished

**What is usable today: everything in 5.3.** The backend is a complete isolation, measurement and
telemetry substrate with a documented, tested observation surface. The game-facing action layer still
needs to be designed, implemented, connected, and validated.
...
**Implementation note:** these handles report the _client's_ view of a session — its lifecycle, its
window, and its health. The codebase does not yet read or influence the game's matchmaking state, so a
matchmaking interface remains to be designed and implemented.
...
### 5.4 Additional unfinished work
...
_This document was produced from the tree at `8420e24`, and is itself committed in `b6f118b` — which is why `git log` shows a HEAD one commit ahead of the hash named in the header. Every figure above was captured before the commit that added this file._
...
_Generated from the working tree at HEAD `8420e24`. Line counts use the ceiling test's own measure
(`content.split('\n').length`)._
...
| `self-test-footprint.cjs`    |   181 | Footprint assertions for the packaged self-test: what a session _claims_ to be, read back out…   |
...
| `target-identity.cjs`        |    75 | The target half of a session footprint: the CDP overrides that make a _live page_ claim a con…   |
